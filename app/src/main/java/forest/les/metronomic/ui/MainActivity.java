...
public class MainActivity extends AppCompatActivity
    implements ItemTouchCallback, ItemFilterListener<SampleItem> {
  ...
  @NonNull private void getDisposable(EditText valute_value) {
  ...
  private void getActualData() {
    ...
    Observable.zip(valutaFullObservable, currentObservable, btcObservable,
        (valuta, valCurs, example) -> {
  ...
  private void getBtcMonthHistory() {
    ...
    apiCoinDesc.getBitcoinRAtesByMonth()
        ...
        }, throwable -> Timber.e(throwable, "getActualData: %s", throwable));
  }

  private void getPeriodsRates() {

    Observable.fromIterable(currentRateList)
        .flatMap(valute -> cbrApi.getPeriodRx(Helper.getMonthAgoTime(), Helper.getActualTime(),
            valute.id).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()))
        .map(valCursPeriod1 -> {
        ...
        }, throwable -> Timber.e(throwable, "getPeriodsRates: %s",
  ...
  public void showValuteRates(List<Valute> currentRateList) {
    ...
    Observable.fromIterable(currentRateList).map(valute -> {
  ...
  @Subscribe public String calculate(String input, Valute inpValute, Valute outValute) {
    ...
    BigDecimal divide = inRubles.divide(outInRubles, BigDecimal.ROUND_HALF_UP)
  ...
  public void updateCalculatorList(int skipPosition, String value, Valute currVal) {
    ...
    Observable.fromIterable(currentRateListRx)
        ...
        .subscribe(rxCalcItem -> {
          ...
          adapter.notifyAdapterItemRangeChanged(skipPosition + 1,
              currentRateListRx.size() - skipPosition);
  ...
  void getBitcois() {
    ...
    Helper.getBtcApi().getData().subscribe(example -> {
  ...
  @Override public boolean itemTouchOnMove(int oldPosition, int newPosition) {
  ...
  @Override public void itemTouchDropped(int oldPosition, int newPosition) {

  }

  @Override public void itemsFiltered(CharSequence constraint, List<SampleItem> results) {
    //
  ...
  public void notifyCalculateAdapter(int editedIndex) {
    ...
    Timber.i("notifyCalculateAdapter: %d", firstVisiblePosition);
    Timber.i("notifyCalculateAdapter: %d", lastVisiblePosition);
  }
